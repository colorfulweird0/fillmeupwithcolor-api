...
export default async function handler(req, res) {
  ...
  if (req.method !== 'POST') return res.status(405).json({ error: 'Method not allowed', version: 'v2' });
  ...
  if (!key || !secret) {
    return res.status(500).json({ error: 'Missing credentials', hasKey: !!key, hasSecret: !!secret });
  }
